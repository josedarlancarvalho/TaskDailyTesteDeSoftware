...
function renderTasks() {
    ...
    tasks.filter(task => !task.completed).forEach(task => {
        ...
        li.innerHTML = `
            ...
            <p>Lembrete às <span id="task-time-${task.id}">${task.time}</span>
                <button onclick="editTime(${task.id})" class="edit-time">✏️</button>
            </p>
            <div class="actions">
...
function editTime(id) {
    const task = tasks.find(task => task.id === id);
    const newTime = prompt('Novo horário da tarefa:', task.time);
...
// Adiciona um ouvinte de eventos para o envio do formulário de tarefas
